...
// as its Markdown parser for several reasons but mainly because work
// on v2 of the BlackFriday https://github.com/russross/blackfriday
// Markdown parser seems to have mostly stalled. Also because goldmark
// creates a CST (which goldmark calls an AST) whereas Blackfriday does
// not yet. Also goldmark is Commonmark-compliant, but note that this
// does not guarantee compliance with LwDITA MDITA and MDITA-XP.
//
// Technical Approach
//
// This package makes its own new types from Go stdlib XML structures,
// so that they get sensible new names and handy methods while retaining
// an ability to be type-cast back to the Golang stdlib equivalents.
//
